...
export const PROVIDER_COSTS_DETAILED = {
  openai: {
    ...
    output: 0.6, // $0.60 per 1M output tokens (GPT-4o mini)
    model: "gpt-4o-mini",
  },
  anthropic: {
  ...
  gemini: {
    ...
    output: 0.3, // $0.30 per 1M output tokens (≤128K context)
...
export const getCheapestProvider = (
  inputTokens: number,
  outputTokens: number,
) => {
  const costs = Object.entries(PROVIDER_COSTS_DETAILED).map(
    ([provider, _]) => ({
      provider,
      cost: calculateProviderCost(
        provider as keyof typeof PROVIDER_COSTS_DETAILED,
        inputTokens,
        outputTokens,
      ),
    }),
...
export const getAllProviderCosts = (
  inputTokens: number = 1000,
  outputTokens: number = 3000,
) => {
  return Object.keys(PROVIDER_COSTS_DETAILED)
    .map((provider) =>
      getProviderInfo(
        provider as keyof typeof PROVIDER_COSTS_DETAILED,
        inputTokens,
...
export const PROVIDER_COSTS = {
  ...
  anthropic: 12.0, // Claude 3.5 Sonnet (blended cost)
