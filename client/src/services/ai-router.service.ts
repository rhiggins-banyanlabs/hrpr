...
export class AIRouterService {
  ...
  async routeRequest(prompt: string, strategy: Strategy): Promise<AIRouterResponse> {
  ...
  async estimateCost(
    ...
    return providerOrder.map(providerName => {
      ...
      return {
        ...
        estimatedCost: cost,
        model: PROVIDER_COSTS_DETAILED[providerName as keyof typeof PROVIDER_COSTS_DETAILED].model
      };
  ...
  getProviderStatus(): Array<{
    ...
    return Object.entries(PROVIDER_COSTS_DETAILED).map(([provider, config]) => ({
      provider,
